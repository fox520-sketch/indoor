...
(() => {
  ...
  function drawNavTelemetryPanel(ctx, wrapEl, viewport) {
    ...
    const lines = [
      ...
      `平均移動速度：${formatSpeed(avgSpeed)}`,
      `距離起點距離：${formatScaleMeters(startDist)}`,
      `目前位置座標：(${fmt(pose.x, 2)}, ${fmt(pose.y, 2)})`
    ];
    ...
    lines.forEach((line, i) => {
      ctx.fillText(line, boxX + paddingX, boxY + paddingY + 13 + i * lineH);
    });
    ctx.restore();
  }
  ...
  function drawTrack() {
    ...
    drawStartToCurrentDistanceBadge(ctx, wrapEl, state.navViewport, "起點→目前位置");
